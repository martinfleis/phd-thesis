...
@marradi1990 uses the term "*typology*" for flat classification models and "*taxonomy* for hierarchical models. However, @bailey1994 suggests to use the same term to distinguish between classification models based on a different criteria. Methodologically, he recognises two key approaches - one is conceptual, for which he uses the term *typology* and other is numerical, named *taxonomy*. Typology is then a conceptual classification, where resulting classes represent concepts, not empirical cases [@bailey1994]. Which means that typologies can be seen as *qualitative classifications*, because there is generally no statistics involved. The land-use case mentioned above is a good example of a typology. On the other side is a taxonomy, which is quantitative classification, with classes being empirical entities in the system proposed by @bailey1994 as well as earlier by @sneath1973. 

This confusion in what *typology* and *taxonomy* mean and what is the criteria used to distinguish between then is present across the literature and is not easy to solve. Furthermore, as with the term classification, *taxonomy* is also used to describe both process and the final result. 
